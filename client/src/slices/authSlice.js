...
import { updateAllNotes } from "./notesSlice"; // 🆕 import
...
const authSlice = createSlice({
  ...
  // Generate new hash + salt for master password
  ...
  const reEncryptedCredentials = credentials.map((item) => {
    ...
    const decryptedUrl = item.url
      ? CryptoJS.AES.decrypt(item.url, oldPassword + item.urlSalt).toString(
          CryptoJS.enc.Utf8
        )
      : "";
  ...
  // 🔥 Also re-encrypt all notes
  updateAllNotes({ oldPassword, newPassword });
},
